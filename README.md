...
### 5. Добавление изменений в локальный репозиторий из удаленного
 ...
 Чтобы  получить изменения из репозитория в своем аккаунте на GitHub, заходим в редактирование, добавляем текст, сохраняем изменения, добавляем коментарий к комиту.
 Переходим в локальный репозиторий и в терминале вводим команду
```
git pull
```
это составная команда она выполняет два действия выкачивает информацию из удаленного репозитория и производит слияние  ветки. При смерживании ветки может произойти конфликт, который можно разрешить выбором из предложенных версий, как это описано в п. 10 настоящей инструкции.

### 6. Репозиторий для **pull request**
`pull request` - запрос на участие в проекте.
Для того, чтобы предложить автору проекта свою версию необходимо:
* В своём аккаунте на GitHub создать копию репозитория автора понравившегося проекта, например, **"AndreyBulgakov19/SCV_Git_PR"** с помощью кнопки **"Fork"**
---
* Скопировать адрес кнопка копии репозитория **"Code"**
---
* Клонировать копию репозитория на локальный компьютер командой `git clone <адрес>`
---
* Создать новую ветку `git branch имя`
---
* Добавить файл с информацией в новую ветку.
---
* Зафиксировать изменения (коммиты) `git commit -am""`
---
* Отправить изменения на GitHub `git push`, `git push --set -upstream origin main`
---
